...
public class PhilSwiftEntity extends AbstractMeme {
	...
	private void registerTargetGoals() {
		this.targetSelector.addGoal(2, (new HurtByTargetGoal(this)).setAlertOthers(PhilSwiftEntity.class));
	}
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficultyIn, MobSpawnType reason, @Nullable SpawnGroupData livingData, @Nullable CompoundTag dataTag) {
		...
		if (random.nextInt(10) < 3) {
			this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(MemeRegistry.FLEX_TAPE.get()));
		}
		return livingData;
	}
