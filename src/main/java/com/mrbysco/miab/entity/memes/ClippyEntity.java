...
public class ClippyEntity extends AbstractMeme {
	...
	protected void dealDamage(LivingEntity entityIn) {
		...
		if (this.hasLineOfSight(entityIn) && this.distanceToSqr(entityIn) < 0.6D * (double) i * 0.6D * (double) i && entityIn.hurt(damageSources().mobAttack(this), (float) 2)) {
	...
	public void tick() {
		...
		if (this.onGround() && !this.wasOnGround) {
			...
			for (int j = 0; j < i * 8; ++j) {
				...
				this.level().addParticle(iparticledata, d0, this.getBoundingBox().minY, d1, 0.0D, 0.0D, 0.0D);
			}
